...
## 2026-08-29 — Fix: every attendance save for a scheduled period 500'd on staging

- **What**: Reported live on staging (`app-suffa-staging`/`api-suffa-staging`): `POST /api/v1/attendance/sync` returned `401` then `500` (the `401` was just a stale token — axios's refresh-token interceptor silently retried and hit the real bug on the second attempt). This broke every attendance save for a class/course with a real scheduled period today — the common case, not an edge case — so this was effectively "attendance is down" on staging.
- **Root cause**: Found via SSH into the Coolify host (`anas@192.168.1.200`) and reading the staging backend container's logs (`docker logs <container> | grep -A40 "Unhandled exception"`), which the frontend never sees — `app/main.py`'s global exception handler always returns a generic `{"detail": "Internal server error"}` to the client and only logs the real traceback server-side. The traceback: `asyncpg.exceptions.StringDataRightTruncationError: value too long for type character varying(120)` inserting into `student_attendance`. The frontend (`attendance.tsx`) builds a student sync/override `idempotency_key` as `{subject_id}:{session_id}:{attendance_date}:{timetable_slot_id}` — three UUIDs (36 chars each) + a date (10) + three colons = **121 characters** whenever a real period is scheduled (the no-period "general" case suffixes `"general"` instead of a slot UUID and fits well within 120). `StudentAttendance.idempotency_key`/`TeacherAttendance.idempotency_key` were both declared `String(120)` — one character short. This never surfaced in the test suite because it runs against in-memory SQLite by default, and **SQLite doesn't enforce declared `VARCHAR(n)` lengths at all** — only real PostgreSQL does, which is exactly what staging (and the local dev Postgres) runs.
- **Fix**: Widened `StudentAttendance.idempotency_key` from `String(120)` to `String(160)` (headroom above the 121-char maximum; `TeacherAttendance`'s equivalent key never includes a slot ID and stays well under 120, so it was left alone) via Alembic migration `c10d1c28657f_widen_student_attendance_idempotency_` (`e1fe6e517f95` → `c10d1c28657f`).
- **Files**: `backend/app/modules/attendance/models.py`, `backend/alembic/versions/c10d1c28657f_widen_student_attendance_idempotency_.py`, `backend/tests/test_attendance_period_enrollment_history.py` (new test).
- **Verified**: Ran the migration up/down/up against the local dev PostgreSQL (`localhost:5432/mms`, the same schema state as staging — confirmed via `SELECT version_num FROM alembic_version` before starting) and confirmed the column width change both ways. Added `test_sync_accepts_the_frontends_full_length_scoped_idempotency_key`, which builds the exact 121-character key the frontend sends and posts it through `/api/v1/attendance/sync`; proved it's a real bug-catching regression (not a tautology) by running it against a dedicated Postgres test database (`mms_test`, via `TEST_DATABASE_URL`/`TEST_DATABASE_ALLOW_RESET=1`) both with the old model (`git stash` on just `models.py`) — **fails**, reproducing the exact production error — and with the fix — **passes**. Full backend suite run on both SQLite and Postgres before/after: identical `42 failed` (Postgres: `47 failed`, an extra 5 environment-only failures unrelated to this fix — not investigated, out of scope) in both cases, all in unrelated files (`test_whatsapp_connection.py`, `test_timetable_scope.py`, `test_username_service.py`, `test_self_service.py`) — this change adds exactly one new passing test and regresses nothing.
- **Deployed to staging DB (2026-08-29, with explicit user approval)**: SSH'd to the Coolify host (`anas@192.168.1.200`, see the `staging-ssh-access` memory), found the running staging backend container (`lcbquzcvzmpgjwwlfqsw8eic-093141750308`, `ghcr.io/anasjadoon31/suffa-ms-backend:staging`) and its database env (`DATABASE_URL` pointing at `suffa_ms_review` on a sibling Postgres container). Confirmed staging was at Alembic revision `e1fe6e517f95` (exactly this migration's `down_revision`) before touching anything. `docker cp`'d the migration file into the running container's `/app/alembic/versions/`, ran `alembic upgrade head` inside it, and confirmed both `alembic current` → `c10d1c28657f (head)` and a direct `information_schema.columns` query showing `idempotency_key` now at 160 characters on the live staging database.
- **Not yet done**: this fix (model change, migration, test) exists only as uncommitted local changes — not committed or pushed. The staging *database* is fixed, but the staging *deployed image* doesn't contain the code yet, so the next staging redeploy needs this branch merged in (the migration is idempotent — `alembic upgrade head` against an already-migrated DB is a no-op). Production hasn't been touched at all. See `TO_IMPLEMENT.md`.

## 2026-08-29 — Fix: offline attendance save could hang forever on "Save offline"

- **What**: Reported live on staging with a screenshot: the "OFFLINE — showing saved data" banner was up (genuinely offline), and the "Save offline · Save 30 marks" button stayed spinning indefinitely — same symptom class as the earlier "attendance save spinner stuck forever" fix in this log, but that fix only covers the *online* path (an axios request that never settles); this report was while already offline, which takes a completely different code path with no equivalent guard.
- **Root cause**: `attendance.tsx`'s offline save path (`if (!online) { for (const entry of entries) { await enqueueEntry(entry); } }`) awaits a Dexie (IndexedDB) write per entry with no timeout. IndexedDB has a well-known failure mode where opening/upgrading a connection can be indefinitely **blocked** — not rejected, just left pending forever — if another connection to the same database (typically a backgrounded PWA tab left open from before a redeploy) is still open. Unlike a rejected promise, a blocked open never settles, so an unguarded `await` on any operation against that database — and whatever UI state is driven by it, here the save button's `isPending` spinner — hangs forever with no error to recover from. `useOnlineStatus`'s `online` state (read via the same shared `window` `online`/`offline` listeners `PwaLayer`'s banner uses) confirmed the offline branch, not the online one, was actually executing.
- **Fix**: 
  1. New `enqueueEntries()` (`outbox.ts`) races the enqueue loop against a 10s timeout, converting an indefinite hang into a clear, retryable `Error` — same defensive pattern as `api.ts`'s existing 30s axios timeout for the online path. `attendance.tsx`'s two call sites (offline branch and the online-attempt's catch fallback) now use it instead of a bare `for` loop.
  2. `MmsDatabase` (`db.ts`) now closes itself on a `versionchange` event (`this.on("versionchange", () => this.close())`) — the standard IndexedDB/Dexie multi-tab pattern. This doesn't unstick an already-affected tab (it already loaded the old JS without this fix), but once every open tab has loaded a build with this handler, a tab holding an old connection will proactively release it the moment a newer tab tries to open a higher schema version, instead of blocking that tab's `db.open()` forever — preventing recurrence for future schema bumps.
- **Files**: `app/src/lib/mms/outbox.ts`, `app/src/lib/mms/db.ts`, `app/src/routes/attendance.tsx`.
- **Verified**: `tsc --noEmit`/`eslint` clean on every touched line (attendance.tsx has ~161 pre-existing, unrelated prettier formatting issues elsewhere in the file, confirmed untouched by this change). Production build unaffected. Not verified: the exact trigger (cross-tab IndexedDB blocking vs. some other IndexedDB stall) wasn't reproduced directly — the fix addresses the whole class of "Dexie operation never settles" bugs regardless of specific cause, converting silent infinite hangs into a visible, actionable error either way.
- **Follow-up if it recurs**: if a user hits the new 10s timeout error repeatedly, that confirms a genuine stall (not just this one screenshot) and is worth a real-device investigation — likely closing other open tabs/windows of the app, or (if on iOS Safari/PWA) a known WebKit IndexedDB reliability issue under memory pressure, tracked separately in `TO_IMPLEMENT.md`'s real-device offline testing item.

## 2026-08-29 — Fix: CI Docker build failure after `recharts` v3 bump (`npm ci` lock-file mismatch)

- **What**: The GitHub Actions/Coolify Docker build for the frontend failed at `npm ci --no-audit --no-fund` (`app/Dockerfile`) with `Invalid: lock file's recharts@2.15.4 does not satisfy recharts@3.10.1` plus several missing/mismatched transitive packages.
- **Root cause**: The recharts v3 bump and `@types/dompurify` removal earlier in this session (see the "PWA Offline Fix Follow-Up" entry below) were installed locally with `pnpm install`, which only updates `pnpm-lock.yaml`. This repo has both lockfiles, but the actual deploy path (`app/Dockerfile`) uses `npm ci`, which requires `package-lock.json` — left stale, still pinned to `recharts@2.15.4`.
- **Fix**: Regenerated `app/package-lock.json` via `npm install --package-lock-only`.
- **Files**: `app/package-lock.json`.
- **Verified**: Copied the regenerated `package.json`/`package-lock.json` into a clean directory and ran `npm ci --no-audit --no-fund` standalone — succeeds (`added 703 packages`), the exact command and lockfile pairing the Docker build step runs. `npm run build` still succeeds in the working tree. Full BuildKit image build not run locally (no `buildx` installed on this machine), but both steps that actually failed/matter in the Dockerfile (`npm ci`, then `npm run build`) are independently confirmed.
- **Lesson for next time**: this repo mixes `pnpm` (used interactively here) and `npm ci` (used in the Docker build) — a `package.json` dependency change needs `package-lock.json` regenerated too (`npm install --package-lock-only`), not just `pnpm-lock.yaml`, or the next CI build breaks even though everything passes locally.

## 2026-08-29 — PWA Offline Fix Follow-Up: four code-fixable items closed

- **What**: Closed the four code-fixable items from the "2026-08-29 — PWA Offline Fix Follow-Up" backlog entry (the other three — live Coolify/Cloudflare deploy verification, a real-device offline smoke test, and confirming the production Redis eviction policy — aren't code and stay open in `TO_IMPLEMENT.md`).
- **`flushMutations()` no longer miscounts a failed replay as "synced"**: `api.ts`'s request and response interceptors both now check for `__offlineReplay: true` on the request config and skip their reactive offline re-queueing when it's set. `mutationQueue.ts`'s `flushMutations()` sets that flag on every replay request it sends via `api.request()`. Previously, a replay that failed with no response (a network hiccup mid-flush) was silently caught by the response interceptor, re-queued as a brand-new entry with a fresh idempotency key and reset attempt count, and resolved as if it had succeeded — so `flushMutations()` saw no error, deleted the original entry, and counted a request that never reached the server as `synced`. It now propagates a real error into `flushMutations()`'s own catch block, which correctly classifies it `failed`/`rejected` via `isPermanentRejection()` and preserves the original entry's attempt history.
- **Attendance calendar cache now invalidates the moment an offline sync completes**: `useOutboxSync.flush()` dispatches a new `mms:outbox-synced` window `CustomEvent` (exported as `OUTBOX_SYNCED_EVENT`) whenever a flush actually synced ≥1 entry. `attendance.tsx`'s `AttendanceBoard` component listens for it and invalidates `["attendance-class-history"]`/`["attendance-student-history"]`. Previously the optimistic offline-save placeholder written into that query cache (see the entry below on "attendance UI showed no change after saving offline") only got replaced by the server's real copy on the query's next natural refetch (month change, remount, manual refresh) — never automatically when sync finished. Both `useOutboxSync` and `useMutationSync` also now expose a `refresh()` so callers can force an immediate badge-count update after an out-of-band action.
- **Rejected offline entries get a per-entry review, not just blanket discard**: new `RejectedEntriesSheet` component (`app/src/components/app/RejectedEntriesSheet.tsx`), opened via a "Review" button that replaced the old immediate "Discard" button on `PwaLayer`'s rejected-entries banner. Lists every rejected attendance/outbox entry with its date, a status corrector (present/absent/leave), the server's error message, and "Discard"/"Save & retry" actions; lists every rejected generic queued mutation with its method/URL/error and "Discard"/"Retry"; keeps a "Discard all" fallback. Backed by new exports `retryRejectedOutboxEntry`/`editAndRetryOutboxEntry` (`outbox.ts`, reuses the deterministic composite idempotency key since only `status` changes) and `retryRejectedMutation` (`mutationQueue.ts`, same key as the original attempt — consistent with how automatic retries already reuse it across attempts). Scope note: editing is limited to attendance `status` — arbitrary payload editing for generic queued mutations (unknown shape per endpoint) is out of scope; those get retry-as-is only. New strings (`Discard`, `Review`, `Retry`, `Couldn't be saved`, `Save & retry`, `Discard all`, etc.) added to both `en.json` and `ur.json`.
- **Package deprecations resolved**: `recharts` bumped `^2.15.4` → `^3.10.1` in `app/package.json`. Its only consumer, `app/src/components/ui/chart.tsx` (a shadcn-style wrapper with zero call sites anywhere in `src` — confirmed via repo-wide grep before touching it), needed two type updates for v3's API: `ChartTooltipContent`'s prop type now picks from `RechartsPrimitive.TooltipContentProps` instead of `React.ComponentProps<typeof RechartsPrimitive.Tooltip>` (v3 moved `payload`/`label`/`active` out of the `<Tooltip>` element's own props into the content-render props), and `ChartLegendContent`'s now picks from `RechartsPrimitive.DefaultLegendContentProps` instead of the top-level `LegendProps` (v3's top-level `LegendProps` Omits `payload`, since that's the `<Legend>` element's own configuration, not what a custom content function receives). Also fixed a `key={item.dataKey}` (v3's `DataKey<any>` includes a function-typed variant, not assignable to React's `Key`) to reuse the already-stringified `key` computed just above it. `@types/dompurify` removed from `devDependencies` — confirmed dompurify 3.4.x ships its own types (`dist/purify.cjs.d.ts`) and neither of its two importers (`RichText.tsx`, `RichTextEditor.tsx`) reference the stub package.
- **Files**: `app/src/lib/mms/api.ts`, `app/src/lib/mms/mutationQueue.ts`, `app/src/lib/mms/outbox.ts`, `app/src/lib/mms/useOutboxSync.ts`, `app/src/lib/mms/useMutationSync.ts`, `app/src/components/app/PwaLayer.tsx`, `app/src/components/app/RejectedEntriesSheet.tsx` (new), `app/src/routes/attendance.tsx`, `app/src/components/ui/chart.tsx`, `app/src/i18n/locales/en.json`, `app/src/i18n/locales/ur.json`, `app/package.json`.
- **Verified**: `tsc --noEmit` — zero new errors; the full remaining error list is byte-identical to the pre-existing baseline in `academics.tsx`, `daily-reports.tsx`, `dashboard.tsx`, `examination.tsx`, `finance.tsx`, `me.tsx`, `people.tsx`, `resources.tsx`, `assignments.tsx`, `PhoneNumbersField.tsx`, and `attendance.tsx:240` (all untouched by this change, all pre-existing per `TO_IMPLEMENT.md`'s own notes). `eslint` clean on every touched file except one pre-existing `as any` at `api.ts:219` (predates this change, part of the token-refresh interceptor, confirmed via `git diff`). `vite build` (client + server + service worker) succeeds. Not verified: browser/device runtime behavior of the new review sheet and the sync-completion invalidation — no running dev server or browser session was exercised in this pass; these are TypeScript/lint/build-verified only.
- **Known scope left for later**: the review sheet's "edit" capability only covers attendance status corrections; a generic payload editor for arbitrary rejected mutations was explicitly left out as disproportionate scope for a per-endpoint-unknown JSON shape.

## 2026-08-29 — Fix: SchemaError querying rejected outbox entries

- **What**: Reported live — `Uncaught (in promise) SchemaError: KeyPath status on object store outbox is not indexed`, thrown from `useOutboxSync.ts`. A separate "change in the order of Hooks" warning in the same report was a Hot Module Reload artifact from the several rapid edits to `PwaLayer.tsx`/`useOutboxSync.ts`/`useMutationSync.ts` in the entry below changing their hook count between saves — confirmed not a real Rules-of-Hooks violation (every hook in all three is called unconditionally, same order, every render) and not addressed with a code change; it resolves with a full page reload, which Fast Refresh's in-place hot-swap can't always do across a hook-shape change.
- **Root cause**: The `outbox` Dexie store's index list is `"++id, idempotency_key, created_at, attempts"` — `status` was never added to it (unlike `mutations`, which has had `status` indexed from the start). The rejected-entry queries added in the entry below used `db.outbox.where("status").equals("rejected")`, an indexed lookup, against a field that isn't an index.
- **Fix**: `getRejectedOutboxEntries`/`getRejectedOutboxCount`/`discardAllRejectedOutboxEntries` now scan with `.toArray()` and filter/delete in memory instead, matching the pattern the rest of `outbox.ts` (`getRecoverableEntries`) already used for this exact reason — no schema/version bump needed, and the outbox is small enough (a handful of queued attendance batches at most) that this costs nothing measurable.
- **Files**: `app/src/lib/mms/outbox.ts`.
- **Verified**: `tsc --noEmit`/`eslint` clean; production build unaffected. Live Playwright run against the real dev server: seeded a synthetic rejected outbox row directly, confirmed no `SchemaError` (or any console/page error) across `getRejectedOutboxCount`/`getRejectedOutboxEntries`/`discardAllRejectedOutboxEntries`, and that a rejected entry is correctly excluded from `outboxCount()` (5/5 checks).

## 2026-08-29 — Offline queue now distinguishes permanent rejections from transient failures

- **What**: Closes the flagged follow-up from the entry directly below — the retry loop in both offline queues couldn't tell "the server permanently refused this" (a 4xx business-rule rejection, like the "no scheduled period" case just fixed) apart from "this failed for a transient reason" (network blip, backend restart), so *any* permanent rejection reproduced the same "N pending sync, never clears" symptom that fix addressed for one specific case.
- **Fix**: Added `isPermanentRejection()` (`mutationQueue.ts`, shared by `outbox.ts`) — true for a response the server actually sent back in the 4xx range, excluding 408 (timeout) and 429 (rate limit), both of which mean "try again," not "this is wrong"; false for no response at all (network failure — stays transient/retryable) and any 5xx. Both `MutationEntry` and `OutboxEntry` gained a `"rejected"` status, permanently excluded from the recoverable/retryable set (unlike `"failed"`, which keeps retrying) — `flushMutations()`'s catch block and `flushOutbox()`'s catch block plus its `locked`/unacknowledged-keys branch (a locked day is exactly this kind of permanent, `/override`-only rejection) now classify into `"rejected"` instead of always `"failed"`.
- **Surfaced in the UI**: `useMutationSync`/`useOutboxSync` now also expose `rejected`/`discardRejected`. `PwaLayer` shows a distinct red "N couldn't be saved — needs review" banner (separate from the amber "pending sync" one, which now only ever reports genuinely-retryable entries) with a Discard action, so a permanent rejection is visible and resolvable instead of silently retried forever or hidden inside a count that implies "will resolve on its own."
- **Files**: `app/src/lib/mms/mutationQueue.ts`, `app/src/lib/mms/outbox.ts`, `app/src/lib/mms/db.ts`, `app/src/lib/mms/useMutationSync.ts`, `app/src/lib/mms/useOutboxSync.ts`, `app/src/components/app/PwaLayer.tsx`.
- **Verified**: `tsc --noEmit`/`eslint` clean; production build unaffected. Live Playwright run against the real dev server driving the actual source modules: `isPermanentRejection` correctly classifies 422/404 as permanent and no-response/429/503/non-axios-error as transient (6/6); a full enqueue → mark-rejected → discard cycle correctly drops the entry out of the pending count immediately, appears in the rejected count with its error message retained, and clears on discard (6/6) — 12/12 total.

## 2026-08-29 — Fix: attendance permanently rejected ("No scheduled period found") for teachers without a timetable slot that day

- **What**: Reported live: marking attendance for a class/course with no scheduled period today failed with `422 No scheduled period found for this course today`, and separately, "6 pending sync which is not syncing." Confirmed as a deliberate product decision before changing anything (asked the user directly, given this touches a data-integrity rule from earlier work, not an obvious bug): a teacher can be assigned to a class as a fixture/backup, with no timetable slot that day or without teaching it at all — attendance should still be recordable as a general (non-period) entry, not rejected outright.
- **Root cause**: `_resolve_entry_period_scope` (`backend/app/modules/attendance/routes.py`) hard-rejects with 422 when `course_id` is given but no matching `TimetableSlot` exists for that class/section/course/day — deliberately added in an earlier period-inference feature (see the 2026-08-01 entries below), but with no fallback for "genuinely no period today," only for "ambiguous, pick one" (multiple periods → 409, handled separately and correctly, untouched by this fix). Every offline-queued entry hitting this is a *permanent* rejection, not a transient failure — the generic outbox retry loop has no way to distinguish the two, so it just kept resending the same doomed entries forever, which is why "pending sync" never cleared.
- **Fix**: When no timetable slot resolves, the entry is now accepted as a general record instead of rejected — both `course_id` and `timetable_slot_id` come back null (not just the slot): the table has a DB check constraint, `(course_id IS NULL) = (timetable_slot_id IS NULL)`, and without a real resolved period there's no concrete class session left to attribute the course to anyway.
- **Files**: `backend/app/modules/attendance/routes.py`, `backend/tests/test_attendance_period_enrollment_history.py` (new test).
- **Verified**: New test posts a sync entry for a day with no matching timetable slot in the seed fixture and confirms `200`/`accepted: 1` with both `course_id` and `timetable_slot_id` null in the stored row. Ran the full backend suite before/after and diffed failing-test sets: byte-identical — the one test in this same file that fails both with and without this change (`test_period_attendance_round_trips_through_roster_sync_and_history`) is a known pre-existing, unrelated regression (confirmed via `git stash`), not something this fix touches.
- **Deployment note**: once shipped, any of the "6 pending sync" entries that were stuck purely on this specific rejection should clear on their very next automatic retry — no client-side change needed, since the same queued payload the frontend already sends is what the backend now accepts.

## 2026-08-29 — Fix: attendance UI showed no change after saving offline

- **What**: Reported live: "Saving offline is not changing the UI" — after tapping Save while offline, the toast appeared but the screen looked exactly as it did before, as if nothing had been marked.
- **Root cause**: `save`'s `onSuccess` unconditionally called `queryClient.invalidateQueries({ queryKey: ["attendance-class-history"] })`. That query is server-backed; an offline save never reaches the server, so the refetch it triggers comes back with the day still showing nothing recorded. Everything downstream reads from that same query — `hasRecords` stays `false`, so `canMark` (`!hasRecords || editing`) stays `true` regardless of the `editing` flag `onSuccess` had just set to `false`, and the "default everyone to present" effect (guarded only by `!canMark || hasRecords`) immediately refills the now-empty `marks` state right back to what it was before saving. The net visible effect: tap Save, toast appears, screen instantly reverts to the same unsaved marking grid.
- **Fix**: An offline save now optimistically writes synthetic `AttendanceLogEntry` rows straight into the `attendance-class-history` query cache via `queryClient.setQueryData` (built from the just-submitted entries + the already-loaded roster, replacing only the entries for the students actually being marked so unrelated rows — e.g. another student's approved-leave record for the same day — aren't clobbered), instead of invalidating that query. The real network sync path is untouched and still invalidates normally once a *real* server response comes back.
- **Files**: `app/src/routes/attendance.tsx`.
- **Verified**: `tsc --noEmit` clean on the touched code (added an explicit return-type annotation to the mutation so the offline/online result branches narrow correctly; one pre-existing unrelated `TS2322` at line 239 untouched). `eslint` clean on the touched lines (two flagged formatting issues nearby both predate this change, confirmed via `git diff`). Production build unaffected.
- **Known follow-up, not fixed here**: once the outbox actually syncs later (back online), nothing currently invalidates this query to replace the optimistic placeholder with the server's own copy — it self-corrects on a fresh mount (new month, remount, manual refresh) but not automatically the moment sync completes. Noted in `TO_IMPLEMENT.md`.
...
- **Root cause**: `AttendanceBoard`'s two "fall back to a valid date when the underlying data no longer contains it" effects (`src/routes/attendance.tsx`, calendar tab and history tab) listed their own `selectedDate` in their dependency arrays. Since the effect's job is to detect "the currently selected date has no matching entry, pick a different one," and *any* click on an unmarked date obviously has no matching entry, the effect re-ran on every manual selection and immediately snapped straight back to `entries[0]` (the first marked date) — a normal user click on any day without existing attendance was instantly overridden, making every unmarked date look permanently unreachable. The intended trigger was a genuine data change (switching months, a refetch), not the user's own selection.
- **Fix**: Removed `selectedDate` from both effects' dependency arrays (with an `eslint-disable-next-line react-hooks/exhaustive-deps` and a comment explaining why) so they only re-run when `history.data`/`studentHistory.data`/`tab` actually change — a real data change can still redirect away from a now-invalid selection, but the user's own click is no longer treated as one.
- **Files**: `app/src/routes/attendance.tsx`.
- **Verified**: `tsc --noEmit` shows no new errors (one pre-existing, unrelated `TS2322` at line 238 predates this change, confirmed via `git diff`). `eslint` clean on the touched lines (the file has ~164 pre-existing, unrelated formatting/lint issues elsewhere, left untouched — not introduced by or in scope of this fix). Production build unaffected.

## 2026-08-29 — Fix: stuck queued login requests hammering the backend rate limiter in production/staging

- **Context**: Observed live on staging (`app-suffa-staging.anas31.qzz.io`): the offline-sync banner stuck at "3 PENDING SYNC" while online, with the console showing a tight loop of `POST /api/v1/auth/token` all returning `429`. Root cause: 3 login requests got queued as offline mutations by the pre-fix version of `api.ts` (see the entry below — its exemption only stops *new* auth requests from being queued going forward; it does nothing for ones already sitting in a browser's IndexedDB from before that fix was live). `flushMutations()` had no awareness that a queued entry might be a credential-bearing auth request, so it kept resending all 3 on every flush cycle, tripping the backend's per-account login lockout (`login_lockout:{tenant}:{username}`, `backend/app/modules/auth/routes.py`) repeatedly. Blast radius: scoped to whichever account was embedded in those 3 stale entries, not IP-wide or tenant-wide — doesn't affect an already-authenticated session, but blocks that one account's fresh logins for a 15-minute window each time the loop fires.
- **Fix**: `flushMutations()` now discards (removes without sending) any claimed entry whose URL matches `/api/v1/auth/`, regardless of how it ended up in the queue — self-healing for a browser that already has a bad entry from before either fix landed, not just prevention of new ones.
- **Files**: `app/src/lib/mms/mutationQueue.ts`.
- **Verified**: `tsc --noEmit`/`eslint` clean. Live Playwright test seeding a synthetic stuck `/api/v1/auth/token` entry directly into IndexedDB (reproducing the staging scenario) and calling `flushMutations()`: entry count drops to 0, zero actual `/api/v1/auth/*` network requests fire during the flush (confirmed via Playwright's request listener, not just the return value).
- **Deployment note**: this only stops the loop once shipped — it doesn't retroactively fix already-affected browsers' IndexedDB from a client-side push alone (the next `flushMutations()` call after the new code loads does that automatically, no manual cache-clearing needed). Not yet deployed to staging as of this entry.

## 2026-08-29 — Fix: dev-mode SW registration failure, login requests wrongly queued offline, dev hydration mismatch from stale SW cache

- **Context**: Surfaced live in the running dev server once the fixes above made offline mode actually active for the first time in dev too (`VITE_ENABLE_OFFLINE` now genuinely reaches both prod and dev builds — previously registration silently never attempted anywhere, so none of these three bugs had ever been reachable before today).
- **Bug 1 (`SW registration failed: ... encountered an error during installation`)**: `vite-plugin-pwa`'s `devOptions` serves the `injectManifest` service worker at a fixed internal dev path, `/dev-sw.js?dev-sw` — not the production filename (`sw-v5.js`). `src/lib/mms/pwa.ts` hardcoded `/sw-v5.js` for both environments, which 404s under `vite dev` (confirmed: `curl http://localhost:8080/sw-v5.js` → 404, falls through to the SPA catch-all). Fixed: `SW_URL = import.meta.env.DEV ? "/dev-sw.js?dev-sw" : "/sw-v5.js"`. Verified live against the running dev server with Playwright: registration now reaches `state: "activated"` with zero console errors.
- **Bug 2 ("stuck on save offline" at the login screen)**: `api.ts`'s pre-emptive offline-queueing check (`if (isMutationRequest(method) && !isOnline())`) had no exclusion for auth paths — a login `POST` made while `navigator.onLine` reports `false` (a real drop, or just an unreliable read right at page load) got silently queued instead of executed, resolving with a fake `{queued: true}` instead of real tokens. The reactive network-failure path added earlier today already excluded `/api/v1/auth/`; the older pre-emptive path never did. Extracted a shared `isQueueExempt(url)` helper and applied it to both. Verified: a `POST /api/v1/auth/token` while offline now correctly rejects with a real network error and is never queued (confirmed 0 queued mutations before and after).
- **Bug 3 (dev-mode React hydration mismatch, `window.$_TSR` tree ... "اردو" vs a `<span className="ar">` wrapper that doesn't exist anywhere in current `src/`)**: The `suffa-pages` NetworkFirst cache (added earlier today) has no way to know dev source just changed — a page cached at one point in a dev session can be served stale (via NetworkFirst's timeout fallback) against a *later*, changed client bundle, and React reports the resulting markup drift as a hydration mismatch. This only became reachable once Bug 1 was fixed (before that, dev-mode registration never succeeded at all, so nothing was ever actually cached in dev). Runtime caching of real pages/assets/API responses is a production-only concern in the first place — dev output changes on every save, so "remember and replay a previous response" actively fights the workflow. Fixed in `src/sw-v5.ts`: the three runtime-caching `registerRoute` calls (pages/assets/API) are now skipped entirely when `import.meta.env.DEV` (precache and the offline-shell `setCatchHandler` fallback stay active — still useful for testing what an offline visit to an uncached route looks like); the `activate` handler additionally purges any `suffa-pages`/`suffa-assets`/`suffa-api-cache` entries left over from *before* this fix whenever running in dev, so an already-affected browser self-heals on the next reload with no manual cache-clearing needed. Verified the `import.meta.env.DEV` branch is fully dead-code-eliminated from the production build (`grep` for it in the built `sw-v5.js` finds nothing) — production still registers all three caches exactly as before.
- **Files**: `app/src/lib/mms/pwa.ts`, `app/src/lib/mms/api.ts`, `app/src/sw-v5.ts`.
- **Verified**: `tsc --noEmit` and `eslint` clean (pre-existing unrelated `any` at `api.ts` untouched). Live Playwright runs against the actual running dev server for all three fixes individually, plus a 4-reload sequence (matching "revisit while the SW controls the page," the scenario that triggered the mismatch) with zero console/page errors. Production build confirmed unaffected: `.output/public/sw-v5.js` still contains `suffa-pages`/`suffa-assets`/`suffa-api-cache` and no trace of the dev-only branch.

## 2026-08-29 — Fix: attendance outbox two-tab race + generic backend idempotency for the offline mutation queue

- **Context**: Closes the two gaps flagged (not fixed) in the entry directly below.
- **Fix 1 (attendance outbox two-tab race)**: `src/lib/mms/outbox.ts`'s `flushOutbox()` resent every row in the table on each call with no per-entry claim — two tabs of the same PWA (or a retry racing an in-flight flush) would both read and resend the same batch. Applied the same atomic-claim pattern already used in `mutationQueue.ts`: `db.ts`'s `OutboxEntry` gained `status`/`syncing_at`; a new `claimOutboxBatch()` atomically transitions every recoverable entry to `"syncing"` inside one Dexie `db.transaction("rw", ...)` before sending, so a concurrent flush sees them already claimed and skips them (IndexedDB serializes conflicting readwrite transactions across tabs, so this holds even though the two calls originate in different tabs). Entries claimed but not acknowledged by the server response (e.g. locked-day rejections) are marked `"failed"` immediately rather than left on `"syncing"` until the 30s staleness window passes. `enqueueEntry`/`peekOutbox`/`incrementAttempts` reworked accordingly; `peekOutbox`/`incrementAttempts` had no external callers and were removed.
- **Fix 2 (no server-side dedup for the generic mutation queue)**: The attendance module already had its own per-entry idempotency (DB unique constraint + existing-record check in `/attendance/sync`), but the generic offline mutation queue (finance, people, forms, etc. — anything routed through `mutationQueue.ts`) sent no idempotency information to the backend at all. A crash between the server committing a replayed write and the client's `removeMutation()` completing had no server-side way to detect a resend as a duplicate. Added `backend/app/core/idempotency.py`: a generic `Idempotency-Key` request guard registered as app middleware (`app.main.py`), backed by Redis (same "fails open if Redis is unreachable" policy as `app/core/rate_limit.py` — not a regression, since that's today's behavior with no header at all). Purely opt-in: a request only engages the guard if it sends the header, so normal interactive traffic (which never sends it) is completely unaffected — confirmed by a dedicated test. Mechanics: `SET key PENDING NX EX 24h` atomically claims a key; a second request for the same key gets the cached response if the first already completed successfully, or a `409` (client's own retry/backoff naturally picks it up next cycle) if the first is still in flight; a failed (non-2xx) outcome is never cached so a legitimate retry actually re-executes. `mutationQueue.ts`'s `flushMutations()` now sends `entry.idempotency_key` as this header on every replay.
- **Files**: `app/src/lib/mms/outbox.ts`, `app/src/lib/mms/db.ts`, `app/src/lib/mms/mutationQueue.ts`; `backend/app/core/idempotency.py` (new), `backend/app/main.py`, `backend/tests/test_idempotency_middleware.py` (new).
- **Verified**: Frontend — `tsc --noEmit` and `eslint` clean (pre-existing unrelated `any` at `api.ts:187` untouched); production build unaffected (runtime-only). Backend — 5 new tests against real Redis (not mocked) covering: duplicate send with the same key creates one record and the second call gets the identical cached response; different keys both execute; no-header traffic is completely unaffected; two concurrent requests with the same key (`asyncio.gather`) create exactly one record; a failed (422) request doesn't poison the key so a corrected retry still executes. Ran the full existing backend suite before and after this change and diffed the failing-test sets: identical except this file's own tests going from failing (feature not wired) to passing — the 42 pre-existing failures elsewhere (`test_whatsapp_connection.py`, `test_admission_conversion.py`, etc.) are unchanged, confirming zero regressions.

## 2026-08-29 — Fix: offline mutation sync reliability (network-failure detection, stuck entries, multi-tab duplicate sends)

- **Context**: Follow-up verification of the offline mutation sync path (`src/lib/mms/mutationQueue.ts`, `useMutationSync.ts`, wired through `src/lib/mms/api.ts`) after the SW fixes above made offline mode actually live for the first time. Verified with a live-browser Playwright harness driving the real source modules (via Vite dev-server ESM, not reimplemented test doubles) against a local mock backend — 19 checks covering queueing, auto-flush-on-reconnect, and two concurrency edge cases. Found and fixed three real gaps:
- **Gap 1 (network failures were silently dropped, not queued)**: `api.ts`'s request interceptor only queued a mutation when `navigator.onLine` was already `false`. `navigator.onLine` reflects the network adapter, not real reachability — a weak signal, VPN hiccup, or an unreachable/down backend all leave it `true` while requests genuinely fail, which is the more common real-world failure mode. Those mutations were just lost with an error toast, never queued, never retried. **Fix**: the response interceptor now also queues any mutation-method request that fails with no `error.response` at all (a real network-level failure, as opposed to a 4xx/5xx the server actually answered), excluding `/api/v1/auth/*` paths. To keep the replayed payload consistent with the existing pre-emptive path, the original (pre-JSON-serialization) request body is now stashed on the config in the first request interceptor (`__offlineReplayData`) — axios has already turned object bodies into a JSON string on `config.data` by the time a failed request reaches the response interceptor, so reading `error.config.data` directly would have queued the wrong shape.
- **Gap 2 (a crash mid-sync made an entry vanish forever)**: `flushMutations()` marks an entry `"syncing"` right before the network call. If the app was killed or reloaded between that mark and the call resolving, the entry was left on `"syncing"` — a status `getPendingMutations()`/`getMutationCount()` didn't query for, so it silently disappeared from both the pending count and the sync banner, never retried, forever. **Fix**: a `"syncing"` entry now counts as recoverable once it's older than 30 seconds (`STALE_SYNC_MS`) — long enough that no real in-flight request could still be running.
- **Gap 3 (two tabs of the same PWA double-submit a queued write)**: `PwaLayer` (and its `useMutationSync()`/`useOutboxSync()` hooks) mounts once per page via the root layout — normal for one tab, but the same PWA open in two tabs each gets its own independent hook instance, both polling and flushing the *same* IndexedDB database (IndexedDB is origin-scoped, not tab-scoped). The `flushingRef` reentrancy guard is per-hook-instance and does nothing across tabs. Caught directly by the test harness: mounting a second `useMutationSync()` instance alongside the app's own real one sent the same queued mutation to the backend twice. **Fix**: `flushMutations()` now atomically claims each entry via a Dexie `db.transaction("rw", ...)` (read current status, only transition to `"syncing"` if it isn't already, in the same transaction) before sending — IndexedDB serializes conflicting readwrite transactions against the same object store across every tab/context with the database open, so this is a real mutual-exclusion guarantee, not just an in-memory flag. The 30s staleness window from Gap 2 disambiguates "another tab is actively sending this right now" (skip) from "a previous attempt crashed" (retry). Verified with two concurrent `flushMutations()` calls racing on one queued entry: the backend received it exactly once.
- **Files**: `app/src/lib/mms/api.ts`, `app/src/lib/mms/mutationQueue.ts`, `app/src/lib/mms/db.ts` (added `syncing_at` to `MutationEntry`).
- **Verified**: `tsc --noEmit` and `eslint` clean on all touched files (remaining project-wide `tsc` errors are pre-existing and unrelated, in `daily-reports.tsx`/`dashboard.tsx`/`examination.tsx`/`finance.tsx`/`incomplete-profiles.tsx`/`me.tsx`/`people.tsx`/`resources.tsx`). Live Playwright suite against the real dev server + a local mock backend: 19/19 checks passed, including the real `useMutationSync()` React hook auto-flushing to the backend with zero manual calls purely from a browser `online` event, and the two-tab race sending a duplicated entry exactly once after the fix (it sent twice before). `npm run build` unaffected (these are runtime-only changes, no build/SW impact).

## 2026-08-29 — Fix: PWA offline navigation crash + all navigations bypassing SSR (`window.$_TSR.h is not a function`)

- **Context**: Follow-up to the same-day fix below. Once that fix actually let the service worker register in a real browser (previously it silently never ran), two further, more serious bugs surfaced that had been dormant precisely because offline mode had never been live before.
- **Root cause 1 (every navigation always bypassed real SSR)**: `generateSW`'s `navigateFallback` option compiles to a `NavigationRoute` that Workbox registers *before* any user-defined `runtimeCaching` route and matches every navigation-mode request unconditionally. Once a route's predicate matches, Workbox commits to that route's handler and never falls through to a later-registered route even on failure. Result: **every full-page navigation, online or offline, was served the static precached shell instead of the real per-route SSR page**, the instant the SW took control (`clientsClaim`). This was invisible during development because offline mode had been silently disabled (see below) — client-side (pushState) navigation never went through the SW's fetch handler at all, only true reloads/deep links did, so it never appeared to break anything.
- **Root cause 2 (offline crash)**: The static app-shell (`generateAppShell` plugin in `vite.config.ts`) hand-stubbed `window.$_TSR = { t: {}, p: function(){}, buffer: [] }` to satisfy TanStack Start's client hydration bootstrap. The real bootstrap script (captured live from an actual SSR response) is `self.$_TSR = { h(){...}, e(){...}, c(){...}, p(e){...}, buffer:[], router: {...} }` — the stub was missing `h`/`e`/`c`/`router` entirely. TanStack Start's client entry unconditionally calls `window.$_TSR.h()` and reads `window.$_TSR.router.manifest` during hydration, so any navigation that hit the shell (which, per root cause 1, was *every* navigation) crashed with `window.$_TSR.h is not a function`. This exact shape was hand-copied once for an older TanStack Start version and had no way to stay in sync with the framework's internal hydration contract — it had already broken once before (2026-08-14 "Invariant failed" entry) and broke again after the 2026-08-23 TanStack version bump.
- **Fix**: Switched the PWA strategy from `generateSW` to `injectManifest` (`app/src/sw-v5.ts`, replacing the dead `src/sw.ts` removed in the entry below) to get full control over route registration order, which `generateSW`'s declarative config cannot express:
  1. `NetworkFirst` for `request.mode === "navigate"` (cache `suffa-pages`, 3s network timeout, denylists `/~oauth`) registered **first**, so real SSR HTML is always tried over the network first and the actual visited page's HTML is cached for offline reuse.
  2. `CacheFirst` for static assets, then `StaleWhileRevalidate` for `/api/v1/`, unchanged from before.
  3. `setCatchHandler` serves the precached `offline.html` **only** when both network and cache fail for a navigation — true last resort, for a route never visited before while offline.
  4. Replaced the hand-faked `$_TSR` app-shell entirely: `offline.html` is now a plain static file in `public/` with no JS bundle, no hydration, no dependency on any TanStack Start internal contract — just a "you're offline" message with a retry button and an `online` event listener that auto-reloads. It cannot break on a future framework upgrade because it doesn't touch framework internals at all. Removed the now-unneeded `generateAppShell` vite plugin.
  5. Renamed the SW output `sw-v4.js` → `sw-v5.js` (source `src/sw-v5.ts`) to bust any CDN/browser cache of the old, broken worker; updated `SW_URL` and the stale-registration cleanup check in `src/lib/mms/pwa.ts` to match.
  6. Added `workbox-precaching`, `workbox-routing`, `workbox-strategies`, `workbox-expiration`, `workbox-cacheable-response` as explicit `devDependencies` (previously only resolved transitively through `vite-plugin-pwa`).
- **Files**: `app/vite.config.ts`, `app/src/sw-v5.ts` (new), `app/public/offline.html` (new, static), `app/src/lib/mms/pwa.ts`, `app/package.json`.
- **Verified**: Full clean `npm run build` (postbuild manifest-size fix still fires correctly for `sw-v5.js`); `tsc --noEmit` and `eslint` clean on all touched files; live Playwright run against the built `node .output/server/index.mjs`: SW registers and activates as `sw-v5.js`; a page visited once while online and then reloaded offline serves the real cached SSR HTML with zero console/page errors (previously crashed with `window.$_TSR.h is not a function`); a route never visited before, navigated to while offline, cleanly renders the static offline page instead of crashing; root `/` still returns real SSR HTML through the SW.

## 2026-08-29 — Fix: PWA offline functionality (truncated SW + silently disabled offline mode)

- **Root cause 1 (truncated service worker)**: recurrence of the known Nitro/client-build race (see 2026-08-14 entries below) that this time hit only the vite-plugin-pwa-generated `sw-v4.js` — Nitro's SSR build snapshots `.output/public` for its embedded asset manifest slightly before the PWA plugin's `generateSW` hook finishes writing the final file, baking a stale `Content-Length` (6058 instead of the real 6401 bytes) into `.output/server/index.mjs`. The server then truncated every response for `/sw-v4.js`, so the browser received a syntactically broken script and service-worker registration failed with a script-evaluation error.
- **Root cause 2 (offline mode silently off)**: `vite.config.ts` gated the whole offline Workbox config (precaching, `navigateFallback`, API/asset caching) behind `process.env.VITE_ENABLE_OFFLINE === "true"`, but Vite never merges `.env` file values into `process.env` for the config file's own Node context (that auto-merge only happens for client `import.meta.env`). `VITE_ENABLE_OFFLINE` was also never set anywhere, so every build — local and Docker — silently took the "offline disabled" branch (0 precached entries, no `navigateFallback`), and `registerServiceWorker()` in `src/lib/mms/pwa.ts` explicitly refuses to register when that flag isn't `"true"`.
- **Fix 1**: Added `app/scripts/fix-public-assets-manifest.mjs`, a small postbuild step (wired as `"postbuild"` in `package.json`, which npm/pnpm/bun all run automatically after `build`) that walks every file actually written under `.output/public` and corrects any stale `size`/`etag`/`mtime` in `.output/server/index.mjs`. Replaces three dead, mutually inconsistent earlier attempts (`scripts/fix-sw.py`, `fix-sw.sh`, `fix-sw-manifest.mjs`) that were never actually invoked by `npm run build` or the Dockerfile — the previous fixes existed only as unwired scripts.
- **Fix 2**: `vite.config.ts` now explicitly calls Vite's `loadEnv()` and merges it into `process.env` before evaluating the workbox config, so `VITE_ENABLE_OFFLINE` set in `.env` actually takes effect. Set `VITE_ENABLE_OFFLINE=true` in `.env`/`.env.example`, and added the same `ARG`/`ENV` passthrough pattern already used for `VITE_API_BASE` in the `Dockerfile` (defaulted to `true`) so Coolify builds enable offline mode without extra configuration, matching the project's env-driven-only mandate.
- **Cleanup**: Removed `src/sw.ts` — dead code left over from an earlier `injectManifest` attempt; the project uses `strategies: "generateSW"`, so vite-plugin-pwa generates the service worker itself and never reads this file.
- **Files**: `app/vite.config.ts`, `app/scripts/fix-public-assets-manifest.mjs` (new), `app/package.json`, `app/Dockerfile`, `app/.env`, `app/.env.example`; removed `app/scripts/fix-sw.py`, `app/scripts/fix-sw.sh`, `app/scripts/fix-sw-manifest.mjs`, `app/src/sw.ts`.
- **Verified**: Clean `npm run build` with only `.env` set (no shell export) precaches 98 entries; postbuild log confirms the `/sw-v4.js` manifest entry was corrected; a full scan of all 101 `.output/public` files against the built `index.mjs` manifest shows zero size mismatches; `node .output/server/index.mjs` boots without error; `curl` against `/sw-v4.js` returns `Content-Length: 6401` matching the actual 6401 downloaded bytes (previously truncated at 6058) and the file passes `node --check`; `/offline.html`, `/manifest.webmanifest`, and the workbox runtime chunk all serve `200`; root `/` still returns real SSR HTML (not the bare shell); the compiled client bundle shows the `VITE_ENABLE_OFFLINE` check dead-code-eliminated to always-register, confirming the flag reaches the client build correctly.
