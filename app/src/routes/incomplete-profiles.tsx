...
function IncompleteProfilesPage() {
  ...
  return <AppShell title={title} subtitle="Complete the required profile details">
    ...
    {query.data?.length ? <><SectionTitle action={<ActionButton onClick={() => remind.mutate()} disabled={remind.isPending}><BellRing className="h-4 w-4" />Send reminder</ActionButton>}>Profiles</SectionTitle><div className="space-y-2.5">{query.data.map((profile) => <Card key={profile.id}><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{profile.name}</p><p className="mt-1 text-xs text-muted-foreground">{profile.phone || "No default phone"}</p></div><Pill tone="warning">Incomplete</Pill></div><p className="mt-3 text-sm text-muted-foreground">Missing: {profile.missing_fields.join(", ")}</p></Card>)}</div></> : null}
  </AppShell>;
}
