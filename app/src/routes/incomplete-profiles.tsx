...
function IncompleteProfilesPage() {
  ...
  return <AppShell title={title} subtitle="Complete the required profile details">
    ...
    {query.data?.length ? <><SectionTitle action={<ActionButton onClick={() => remind.mutate()} disabled={remind.isPending}><BellRing className="h-4 w-4" />Send reminder</ActionButton>}>Profiles</SectionTitle><div className="space-y-2.5">{query.data.map((profile) => {
      ...
      return type === "student" ? <Link key={profile.id} to="/people/$studentId" params={{ studentId: profile.id }} className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{card}</Link> : <Link key={profile.id} to="/guardians/$guardianId" params={{ guardianId: profile.id }} className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{card}</Link>;
    })}</div></> : null}
  </AppShell>;
}
