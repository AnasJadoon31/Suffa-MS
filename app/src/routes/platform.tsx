...
function MadrasaCard({ madrasa }: { madrasa: MadrasaEntry }) {
  ...
  const features = useQuery({ queryKey: ["platform", "features", madrasa.id], queryFn: () => api.get<MadrasaFeature[]>(`/api/v1/platform/madaris/${madrasa.id}/features`).then((r) => r.data) });
  ...
  return <Card className="space-y-3"><div className="flex items-start justify-between gap-3"><div><p className="font-display font-extrabold">{madrasa.name}</p>{editingSlug ? <div className="mt-2 flex gap-2"><TextInput value={slug} onChange={(event) => setSlug(event.target.value)} /><ActionButton onClick={() => saveSlug.mutate()} disabled={saveSlug.isPending}>Save</ActionButton></div> : <p className="mt-1 text-xs text-muted-foreground">{madrasa.slug}</p>}</div><div className="flex gap-2"><button type="button" aria-label="Edit slug" title="Edit slug" onClick={() => setEditingSlug((value) => !value)} className="grid h-10 w-10 place-items-center rounded-xl bg-muted"><Pencil className="h-4 w-4" /></button><button type="button" aria-label="Open madrasa workspace" title="Open madrasa workspace" onClick={openWorkspace} className="grid h-10 w-10 place-items-center rounded-xl bg-primary text-primary-foreground"><ExternalLink className="h-4 w-4" /></button></div></div><div><p className="mb-2 text-xs font-bold uppercase tracking-widest text-muted-foreground">Screens</p>{features.isLoading ? <SkeletonList rows={2} /> : <div className="grid grid-cols-2 gap-2">{features.data?.map((feature) => <label key={feature.key} className="flex items-center justify-between gap-2 rounded-xl bg-muted px-3 py-2 text-sm font-semibold"><span>{feature.label}</span><input type="checkbox" checked={feature.enabled} onChange={(event) => toggle.mutate({ ...feature, enabled: event.target.checked })} className="h-4 w-4 accent-primary" /></label>)}</div>}</div></Card>;
}
