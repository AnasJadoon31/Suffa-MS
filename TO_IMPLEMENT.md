...
- The attendance calendar's optimistic offline-save update (`attendance.tsx`) writes synthetic history entries into the `attendance-class-history` query cache, but nothing currently invalidates that query when the outbox actually syncs later — the placeholder only gets replaced by the server's real copy on a fresh query mount (month change, remount, manual refresh), not automatically the moment sync completes. Wire `useOutboxSync`'s flush success (or a generic "sync completed" event) to invalidate the relevant query keys.
- Discarding a rejected entry (the new "N couldn't be saved — needs review" banner in `PwaLayer`) only removes it — there's no way to *edit and resubmit* it yet (e.g. a mis-scoped attendance entry the user could correct and requeue instead of losing the mark entirely). Worth a review sheet listing rejected entries with their server error, offering discard or edit-and-retry per entry, instead of only a blanket "discard all."
- `api.ts`'s response interceptor's reactive offline-queueing (for a mutation that fails with no response at all) runs on *every* `api.request()` call, including the ones `flushMutations()` itself makes while replaying a queued entry. If that replay attempt also fails with a network error, the interceptor queues a *fresh* copy (new idempotency key, reset attempt count) and resolves successfully, so `flushMutations()` sees no error, removes the original entry, and counts it as "synced" even though nothing actually reached the server. Not a duplicate-send (the request never succeeded either time) or data loss, but it does lose the original attempt history and reset the retry count on every network hiccup during a replay, and slightly undercounts real failures in the returned stats.
