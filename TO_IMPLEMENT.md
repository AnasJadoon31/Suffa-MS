...
## 2026-08-29 — Offline attendance save could hang forever ("Save offline" spinner stuck)

- DONE: `attendance.tsx`'s offline save loop (`for entry of entries: await enqueueEntry(entry)`) had no timeout — if the local IndexedDB connection is ever blocked (most likely cause: a backgrounded PWA tab from before a redeploy still holding an older connection open), the `await` never settles and the save button spins forever with no error. New `enqueueEntries()` (`outbox.ts`) races the enqueue loop against a 10s timeout; `MmsDatabase` (`db.ts`) now releases its connection on `versionchange` so it stops blocking a newer tab's connection in the first place. Full details in `IMPLEMENTED.md`.
- OPEN — needs real-device confirmation, not more code right now: the exact trigger wasn't reproduced directly (no way to inspect the reporting device's browser/tab state remotely) — this fix converts the failure mode from "silent infinite hang" to "clear 10s-timeout error, retryable," which is correct regardless of cause, but doesn't prove the cross-tab-blocking theory. If the same account hits the new timeout error again, that confirms a real, recurring stall worth a deeper device-specific investigation (folds into the existing real-device offline smoke test item below).
- OPEN — not committed/pushed yet, same as the other 2026-08-29 entries above.
...
- DONE (2026-08-29): The attendance calendar's optimistic offline-save cache no longer goes stale after sync. `useOutboxSync.flush()` dispatches a `mms:outbox-synced` window event whenever a flush actually synced ≥1 entry; `attendance.tsx`'s `AttendanceBoard` listens for it and invalidates `attendance-class-history`/`attendance-student-history` the moment sync completes, not just on next natural refetch.
- DONE (2026-08-29): Rejected offline entries now get a per-entry review flow instead of only blanket "discard all." New `RejectedEntriesSheet` (opened via a "Review" button on the existing rejected-entries banner in `PwaLayer`) lists each rejected attendance/outbox entry (with a status corrector + "Save & retry") and each rejected generic mutation (with "Retry"), plus per-entry Discard and a "Discard all" fallback. Backed by new `retryRejectedOutboxEntry`/`editAndRetryOutboxEntry` (`outbox.ts`) and `retryRejectedMutation` (`mutationQueue.ts`). New UI strings added to `en.json`/`ur.json`.
- DONE (2026-08-29): Package deprecations resolved. `recharts` bumped `^2.15.4` → `^3.10.1`; the only consumer (`components/ui/chart.tsx`, previously unused/dead code with zero call sites) updated for v3's `TooltipContentProps`/`DefaultLegendContentProps` types. `@types/dompurify` removed from `devDependencies` — confirmed dompurify 3.4.x ships its own types (`dist/purify.cjs.d.ts`) and nothing imports the stub package directly. `tsc --noEmit` shows no new errors beyond the pre-existing unrelated baseline (`academics.tsx`, `daily-reports.tsx`, `dashboard.tsx`, `examination.tsx`, `finance.tsx`, `me.tsx`, `people.tsx`, `resources.tsx`, `assignments.tsx`, `PhoneNumbersField.tsx`, `attendance.tsx:240` — tracked separately below); `vite build` succeeds.
- OPEN — needs a real device, not code: Deploy and verify against the live Coolify/Cloudflare stack: confirm the CDN doesn't serve a cached copy of the old `sw-v4.js` (browsers auto-detect the new `sw-v5.js` URL and update since it's a different file, but check Cloudflare edge cache behavior on first real deploy per the 2026-08-14 CDN-cache incidents in `IMPLEMENTED.md`).
- OPEN — needs a real device, not code: Run a real-device offline smoke test (not just Playwright's `context.setOffline`) covering: airplane mode mid-session and force-quitting/reopening the installed PWA while offline.
- OPEN — needs a real device, not code: Confirm on an actual device/deploy that a page reload after a queued offline mutation shows the queued state consistently rather than a stale pre-mutation `suffa-pages` NetworkFirst cache. The specific gap this covered (cache invalidation on sync completion) is fixed above; this item is now about confirming it live.
- OPEN — needs a production-ops decision, not code: The `Idempotency-Key` backend guard (`backend/app/core/idempotency.py`) is Redis-backed with a 24h TTL — if Redis is ever run without persistence/with eviction under memory pressure in production, a claimed-but-not-yet-cached key could theoretically be evicted before the response is stored, losing dedup protection for that one retry. Not a correctness risk (fails open, same as today), just worth confirming the production Redis eviction policy doesn't make this common in practice.
